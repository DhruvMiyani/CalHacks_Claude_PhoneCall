...
# Import Gmail configuration
try:
    from gmail_config import SENDER_EMAIL, APP_PASSWORD, RECIPIENT, POLL_SECS
    GMAIL_AVAILABLE = True
except ImportError:
    print("⚠️  Gmail config not found - email_agent will be disabled")
...
class ClaudeMonitor:
    def __init__(self):
        ...
        self.watch_directory = "/Users/dhruvmiyani/.claude/projects/-Users-dhruvmiyani-sandwich-berkeley-hacks"
    ...
    async def email_agent(self, context: str, screenshots_needed: bool = True) -> str:
        """Send an email with screenshots of the top 3 design choices."""
        if not GMAIL_AVAILABLE:
            return "Email agent unavailable - gmail_config.py not found"
        
        # Pause monitoring during email sending
        print(f"📧 PAUSING monitoring for email sending...")
        self.paused = True
        
        try:
            # Find screenshot files (assuming they're in screenshots directory)
            screenshot_dir = "screenshots"
            screenshot_files = []
            
            if screenshots_needed and os.path.exists(screenshot_dir):
                # Look for recent screenshot files
                pattern = os.path.join(screenshot_dir, "*.png")
                all_screenshots = glob.glob(pattern)
                # Sort by modification time and take the 3 most recent
                recent_screenshots = sorted(all_screenshots, key=os.path.getmtime, reverse=True)[:3]
                screenshot_files = recent_screenshots
                print(f"📸 Found {len(screenshot_files)} screenshots to attach")
            
            # Create email
            subject = f"Design Choices - {uuid.uuid4().hex[:8]}"
            msg_id = f"<{uuid.uuid4().hex}@{SENDER_EMAIL.split('@')[1]}>"
            ...
            # Email body
            email_body = f"""Hi! Here are the top 3 design choices generated by the system.

Context: {context[:500]}{"..." if len(context) > 500 else ""}

Please review the attached screenshots and provide your feedback.

Generated at: {time.strftime('%Y-%m-%d %H:%M:%S')}
Message ID: {msg_id}

Best regards,
CalHacks Design System
"""
            ...
            success_msg = f"✅ Email sent successfully! Subject: {subject}"
            print(success_msg)
    ...
    async def call_claude_for_analysis(self, context_lines: List[str]):
        ...
        tools = [
            ...
            {
                "name": "email_agent",
                "description": "Send an email with screenshots of the top 3 design choices to the configured recipient",
                "input_schema": {
                    "type": "object",
                    "properties": {
                        "reason": {
                            "type": "string",
                            "description": "The reason for sending the email (e.g., 'design choices ready for review')"
                        },
                        "context": {
                            "type": "string",
                            "description": "The relevant context about the design choices to include in the email"
                        },
                        "include_screenshots": {
                            "type": "boolean",
                            "description": "Whether to include screenshot attachments",
                            "default": True
                        }
                    },
                    "required": ["reason", "context"]
        ...
        try:
            response = self.client.messages.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"Context from recent conversation:\n{context_text}\n\n. There will be many logs but focus mostly on the last log as this is the most recent log. Use the previous logs to get an understanding of the conversation. In the last log, if there is an error or question that needs to be answered by the user or multiple plans proposed, then call the voice_agent tool. If design choices or screenshots have been generated and are ready for review, call the email_agent tool to send them to the user."
                    }
                ]
            )
    ...
    async def process_latest_file(self):
        ...
        if hasattr(response, 'content') and response.content:
            for content_block in response.content:
                ...
                elif content_block.type == "tool_use" and content_block.name == "email_agent":
                    # Extract tool parameters
                    tool_input = content_block.input
                    reason = getattr(tool_input, "reason", "") if hasattr(tool_input, "reason") else ""
                    context = getattr(tool_input, "context", "") if hasattr(tool_input, "context") else ""
                    include_screenshots = getattr(tool_input, "include_screenshots", True) if hasattr(tool_input, "include_screenshots") else True
                
                    print(f"📧 CLAUDE DETECTED DESIGN CHOICES - Sending email")
                    print(f"Reason: {reason}")
                    print(f"Context: {context[:100]}..." if len(context) > 100 else f"Context: {context}")
                    
                    # Call the email agent
                    email_response = await self.email_agent(context, include_screenshots)
                    print(f"📧 Email agent response: {email_response}")
        ...
        if not voice_call_made and not email_sent:
